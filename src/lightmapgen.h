...
protected:
	...
	static void CalcEdgeVectors(const CPlane3D& plane, const float* uvMin, const float* uvMax, CPoint3D& edge1, CPoint3D& edge2,
			CPoint3D& UVVector);

	static void NormalizeLightmapUVs(std::vector<CPoint3D>& polyPoints, float* minimums, float* maximums, uint16_t *polyUWidth,
